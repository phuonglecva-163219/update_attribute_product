...
class LeCartCustom(LeBasecart):
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'attributes': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_product_attribute as pattr,  _DBPRF_attribute as attr WHERE attr.attribute_id = pattr.attribute_id AND product_id IN " + product_id_con,
			},
	...
	def convert_product_export(self, product, products_ext):
		...
		product_attributes = []
		for attribute in products_ext['data']['attributes']:
			if attribute['product_id'] == product['products_id']:
	...
	def convert_order_export(self, order, orders_ext):
		...
		subtotal_details = get_row_from_list_by_field(order_rows_id, 'class', 'ot_subtotal')
