...
class LeCartWoocommerce(LeCartWordpress):
	...
	def check_product_import(self, convert, product, products_ext):
		post_id =  self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'])
		if post_id:
			...
			for product_attribute in product_attributes:
				...
				new_att_id = 0
				if not self.get_map_field_by_src(self.TYPE_ATTR, product_attribute['attribute_id']):
					new_att_id = self.import_data_connector(self.create_insert_query_connector("woocommerce_attribute_taxonomies", {
				...
				if not self.get_map_field_by_src(self.TYPE_ATTR_VALUE, code_src = product_attribute['value']):
					...
					wp_term_taxonomy_data = {
						'term_taxonomy_id': attribute_id,
						'term_id': attribute_id,
						'taxonomy': 'pa_' + attr_code,
						'description': '',
						'parent': 0,
						'count': 0
					}
					term_meta_data = {
						'term_id': attribute_id,
						'meta_key': 'order_pa_' + attr_code,
						'meta_value': 0
					}

					self.import_data_connector(self.create_insert_query_connector("term_taxonomy", wp_term_taxonomy_data), "term_taxonomy")
					self.import_data_connector(self.create_insert_query_connector("termmeta", term_meta_data), "termmeta")
					self.insert_map(self.TYPE_ATTR_VALUE, product_attribute['attribute_id'], attribute_id, code_src = product_attribute['value'])
				# self.insert_map(self.TYPE_ATTR_VALUE, product_attribute['attribute_id'], attribute_id)
				else:
					attribute_id = self.get_map_field_by_src(self.TYPE_ATTR_VALUE, code_src = product_attribute['value'])
				# 	wp_term_taxonomy insert
				# insert term realationship
			...
			self.update_product_attribute_in_postmeta(post_id)
			return True
		return False
		# print(product_id)
	...
	def create_transient_wc_attribute_taxonomies(self,id,  attribute_id, attribute_name, attribute_label, attribute_type, attribute_orderby):
		return 'i:'+ str(id) +';' \
		       'O:8:"stdClass":6:{s:12:"attribute_id";s:1:"'+attribute_id+'";' \
		       's:14:"attribute_name";s:'+str(len(attribute_name))+':"'+attribute_name+'";' \
		       's:15:"attribute_label";s:'+str(len(attribute_label))+':"'+attribute_label+'";' \
		       's:14:"attribute_type";s:'+str(len(attribute_type))+':"'+attribute_type+'";' \
		       's:17:"attribute_orderby";s:'+str(len(attribute_orderby))+':"'+attribute_orderby+'";' \
		       's:16:"attribute_public";s:1:"0";}'



	def update_transient_wc_attribute_taxonomies(self):
		transient_wc_attribute_taxonomies_qr = {
			'type': 'select',
			'query': "SELECT _DBPRF_options.option_name FROM _DBPRF_options"
		}
		taxonomies = self.select_data_connector(transient_wc_attribute_taxonomies_qr, 'attribute_taxonomies')['data']
		print(taxonomies)
		list_option_name = [taxonomy['option_name'] for taxonomy in taxonomies]
		if '_transient_wc_attribute_taxonomies' not in list_option_name:
			self.import_data_connector(self.create_insert_query_connector("options", {
		...
		get_list_attributes = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_woocommerce_attribute_taxonomies"
		}

		attributes = self.select_data_connector(get_list_attributes, 'attribute_taxonomies')['data']
		options_value = 'a:'+str(len(attributes))+':{'
		for i in range(len(attributes)):
			attribute = attributes[i]
			options_value += self.create_transient_wc_attribute_taxonomies(
				i, attribute['attribute_id'], attribute['attribute_name'], attribute['attribute_label'],
				attribute['attribute_type'], attribute['attribute_orderby']
			)
		options_value += '}'
		data = {
		...
		update_query = self.create_update_query_connector('options', data, where_id)
		self.import_data_connector(update_query, 'options')

	def update_product_attribute_in_postmeta(self, product_id):
		list_productmeta_query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_postmeta where post_id = {}".format(product_id)
		}
		product_metas = self.import_data_connector(list_productmeta_query, "postmeta")
		meta_title = [meta['meta_key'] for meta in product_metas]

		if '_product_attributes' not in meta_title:
		...
		product_relation_query  = {
			'type':'select',
			'query': 'SELECT * FROM _DBPRF_term_relationships as wtr, _DBPRF_term_taxonomy as tt WHERE wtr.term_taxonomy_id = tt.term_taxonomy_id '\
			         + 'AND wtr.object_id = {}'.format(product_id)
		}
		product_relations = self.import_data_connector(product_relation_query, "termrealations")
		product_attributes = [pr for pr in product_relations if pr['taxonomy'].startswith("pa_")]

		options_value = 'a:' + str(len(product_attributes)) + ':{'
		attrbute_added = []
		for i in range(len(product_attributes)):
			attribute = product_attributes[i]
			if attribute['taxonomy'] in attrbute_added:
				continue
			options_value += 's:'+str(len(attribute['taxonomy']))+':"'+attribute['taxonomy']+'";a:6:{s:4:"name";s:'+str(len(attribute['taxonomy']))+':"' + attribute['taxonomy'] + '";s:5:"value";s:0:"";s:8:"position";i:' + str(i) + ';s:10:"is_visible";i:1;s:12:"is_variation";i:0;s:11:"is_taxonomy";i:1;}'
			attrbute_added.append(attribute['taxonomy'])
		options_value += '}'
		data = {
			'meta_value': options_value
		}
		where_id = {
			'meta_key': '_product_attributes',
			'post_id': product_id
		}
		update_query = self.create_update_query_connector('postmeta', data, where_id)
		self.import_data_connector(update_query, 'postmeta')
		print(options_value)
	...
	def product_import(self, convert, product, products_ext):
		...
		# list_map = self.select_map(self._migration_id, self.TYPE_ATTR_VALUE, code_src=product_attribute['value'])
		#
		# print(list_map)
		self.update_transient_wc_attribute_taxonomies()

		# query = {
		# 	'type': 'select',
		# 	'query': "SELECT * FROM _DBPRF_options where _DBPRF_options.option_name = '_transient_wc_attribute_taxonomies'"
		# }
		# orders = self.select_data_connector(query, 'options')

		# term_id = self.import_data_connector(self.create_insert_query_connector("terms",{
		# 	'name':  attr_name,
		# 	'slug': attr_code,
		# 	'term_group':0
		# }), "term")
		...
		# 	attr_id = self.import_data_connector(self.create_insert_query_connector("terms", {
		# 		'name': attr['value'],
		# 		'slug': self.sanitize_title(attr['value']),
		# 		'term_group': 0
		# 	}), "term")
		#
		# 	self.import_data_connector(self.create_insert_query_connector("termmeta", {
		# 		'term_id':attr_id,
		# 		'meta_key':'pa_' + attr_code,
		# 		'meta_value':0
		# 	}), "termmeta")
		# import attribute
		# import attribute
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		# import category for product

		category_id_import = self.get_map_field_by_src(self.TYPE_CATEGORY, convert['categories'][0]['id'])
		id = self.import_data_connector(self.create_insert_query_connector("term_relationships", {
			'object_id': product_id,
			'term_taxonomy_id': category_id_import,
			'term_order': '0'
		}), "term_relationships")
	...
	def get_total(self, convert):
		sum = 0
		sum += int(float(convert['shipping']['amount']))
		for item in convert['items']:
			sum += int(item['qty']) * float(item['original_price'])
		return sum
	...
	def order_import(self, convert, order, orders_ext):
		...
		# import wp_woocommerce_order_itemmeta
		meta_key_item_order = {
			'cost':convert['shipping']['amount'],
			'total_tax':'0',
			'taxes':'a:1:{s:5:"total";a:0:{}}'
		}
		for key, value in meta_key_item_order.items():
			self.import_data_connector(self.create_insert_query_connector("woocommerce_order_itemmeta", {
				"meta_key": key,
				"meta_value": value,
				"order_item_id": shipping_id
			}),"woocommerce_order_itemmeta")
		...
		for item in convert['items']:
			...
			item_id = self.import_data_connector(qr, 'woocommerce_order_items')
		#
		# self.insert_map(self.TYPE_ORDER, convert['id'], order['orders_id'], convert['code'])
			order_item_meta = {
				...
				'_qty':item['qty'],
				'_tax_class':'',
				'_line_subtotal': int(item['qty']) * float(item['original_price']),
				'_line_subtotal_tax':'0',
				'_line_total': int(item['qty']) * float(item['original_price']),
				'_line_tax':'0',
